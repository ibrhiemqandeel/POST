...
@section('page-sub')<span id="supCount">— مورد — دعم موردين من دول مختلفة (Multi-Supplier)</span>@endsection
  ...
  <div class="overlay" id="overlay">
    <div class="modal">
      ...
      <div class="modal-body">
        ...
        <div class="field full"><label>اسم المورد</label><input id="f_name" placeholder="مثال: Como Textiles Co."></div>
        <div class="field"><label>الدولة</label><input id="f_country" placeholder="مثال: الصين / تركيا"></div>
        <div class="field">
          <label>المنصة</label>
        ...
        <div class="field"><label>البريد الإلكتروني</label><input id="f_email" type="email" placeholder="supplier@example.com"></div>
        <div class="field"><label>الهاتف</label><input id="f_phone" placeholder="+90..."></div>
        <div class="field"><label>العملة</label><input id="f_currency" placeholder="USD" value="USD"></div>
        <div class="field"><label>تكلفة الشحن الافتراضية ($)</label><input id="f_ship_cost" type="number" min="0" step="0.01" placeholder="0.00"></div>
        <div class="field"><label>أقل مدة شحن (أيام)</label><input id="f_days_min" type="number" min="0" placeholder="7"></div>
        <div class="field"><label>أقصى مدة شحن (أيام)</label><input id="f_days_max" type="number" min="0" placeholder="15"></div>
        <div class="field">
          <label>الحالة</label>
          <select id="f_active"><option value="1">مفعّل</option><option value="0">موقوف</option></select>
        </div>
        <div class="field full"><label>ملاحظات</label><input id="f_notes" placeholder="ملاحظات داخلية عن المورد…"></div>
      </div>
...
function deleteSupplier(id){
  ...
  apiRequest(`{{ url('/admin/suppliers') }}/${id}`, { method:'DELETE' })
    .then(() => loadSuppliers()).catch(err => alert(err.message));
}
