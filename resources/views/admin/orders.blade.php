...
@section('page-sub')<span id="orderCount">— طلب</span>@endsection
...
const STATUS_LABELS = {
  pending:'قيد الانتظار', processing:'قيد المعالجة', shipped:'تم الشحن', delivered:'تم التوصيل', cancelled:'ملغي',
};
const STATUS_BADGE_CLASS = {
  pending:'badge-pending', processing:'badge-info', shipped:'badge-info', delivered:'badge-synced', cancelled:'badge-out',
};

function loadOrders(){
  return apiRequest('{{ route("admin.orders.index") }}')
    .then(data => { ORDERS = data.orders || []; renderStats(); render(); })
    .catch(err => {
...
function render(){
  ...
  tbody.innerHTML = filtered.map(o => `
    <tr>
      <td>#${o.id}</td>
      <td><div class="prod-name">${escapeHtml(o.customer_name)}</div><div class="prod-cat">${escapeHtml(o.customer_email)}</div></td>
      <td>${o.items_count}</td>
      ...
      <td>${o.date}</td>
      <td class="row-actions"><button class="icon-btn" title="عرض التفاصيل" onclick="viewOrder(${o.id})">👁</button></td>
    </tr>`).join('');
}

function changeStatus(orderId, status){
  apiRequest(`{{ url('admin/orders') }}/${orderId}/status`, { method:'PATCH', body:JSON.stringify({ status }) })
    .then(() => { const idx = ORDERS.findIndex(o => o.id === orderId); if(idx > -1) ORDERS[idx].status = status; renderStats(); })
            ...
            <table>
              ...
              <tbody>
                ${(o.items || []).map(it => `
                  ...
                  <td class="price-cell">$${Number(it.total).toFixed(2)}</td></tr>`).join('')}
              </tbody>
            </table>
          </div>
          <div style="display:flex;justify-content:space-between;font-weight:700;font-size:16px;padding-top:8px"><span>الإجمالي</span><span>$${Number(o.total).toFixed(2)}</span></div>
        </div>`;
